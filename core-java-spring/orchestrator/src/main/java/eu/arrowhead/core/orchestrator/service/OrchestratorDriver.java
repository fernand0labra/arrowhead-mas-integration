...
import eu.arrowhead.common.dto.shared.TranslatorRequestDTO;
import eu.arrowhead.common.dto.shared.TranslatorResponseDTO;
...
public class OrchestratorDriver {
	...
	private static final String MISMATCH_ANALYSIS_URI_KEY = CoreSystemService.MISMATCHANALYSIS_ANALYSIS_SERVICE.getServiceDefinition() + CoreCommonConstants.URI_SUFFIX;
	
	/* ******************************************************************************************************** */
	/*                                               translator													*/
	/* ******************************************************************************************************** */
	
	private static final String TRANSLATOR_URI_KEY = CoreSystemService.TRANSLATOR_SERVICE.getServiceDefinition() + CoreCommonConstants.URI_SUFFIX;
	
	/* ******************************************************************************************************** */
	/*                                       interface-generation-system 										*/
	/* ******************************************************************************************************** */
	
	private static final String INTERFACE_GENERATOR_URI_KEY = CoreSystemService.INTERFACEGENERATOR_GENERATE_SERVICE.getServiceDefinition() + CoreCommonConstants.URI_SUFFIX;
	...
	public AnalysisResponseDTO analyseServiceRequest(final AnalysisRequestDTO request) {
		...
		final UriComponents analysisUri = getMismatchAnalysisUri();		
	...
	//-------------------------------------------------------------------------------------------------
	public TranslatorResponseDTO createHubServiceRequest(final TranslatorRequestDTO request) {
		logger.debug("createHubServiceRequest started...");
		Assert.notNull(request, "TranslatorRequestDTO is null.");
		
		final UriComponents translatorUri = getTranslatorUri();
		final ResponseEntity<TranslatorResponseDTO> response = httpService.sendRequest(translatorUri, HttpMethod.POST, TranslatorResponseDTO.class, request);
	...
	//-------------------------------------------------------------------------------------------------
	public GenerationResponseDTO generateInterfaceRequest(final GenerationRequestDTO request) {
		logger.debug("generateInterfaceRequest started...");
		Assert.notNull(request, "GenerationRequestDTO is null.");
		
		final UriComponents generatorUri = getInterfaceGeneratorUri();
		final ResponseEntity<GenerationResponseDTO> response = httpService.sendRequest(generatorUri, HttpMethod.POST, GenerationResponseDTO.class, request);
	...
	private UriComponents getMismatchAnalysisUri() {
		...
		if (arrowheadContext.containsKey(MISMATCH_ANALYSIS_URI_KEY)) {
			try {
				return (UriComponents) arrowheadContext.get(MISMATCH_ANALYSIS_URI_KEY);
			} catch (final ClassCastException ex) {
	...
	//-------------------------------------------------------------------------------------------------
	private UriComponents getTranslatorUri() {
		logger.debug("getTranslatorUri started...");
		
		if (arrowheadContext.containsKey(TRANSLATOR_URI_KEY)) {
			try {
				return (UriComponents) arrowheadContext.get(TRANSLATOR_URI_KEY);
			} catch (final ClassCastException ex) {
				throw new ArrowheadException("Orchestrator can't find Translator URI.");
			}
		}
	...
	//-------------------------------------------------------------------------------------------------
	private UriComponents getInterfaceGeneratorUri() {
		logger.debug("getTranslatorUri started...");
		
		if (arrowheadContext.containsKey(INTERFACE_GENERATOR_URI_KEY)) {
			try {
				return (UriComponents) arrowheadContext.get(INTERFACE_GENERATOR_URI_KEY);
			} catch (final ClassCastException ex) {
				throw new ArrowheadException("Orchestrator can't find Interface Generator URI.");
			}
		}
